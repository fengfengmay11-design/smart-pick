// AUTO-GENERATED from database/v2 via migration/build-data-js.js — DO NOT EDIT BY HAND
const PHONE_DATA = [
  {
    "id": "iphone-16",
    "brand": "苹果",
    "model": "iPhone 16",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2016%20Black.jpg",
    "launchDate": "2024-09",
    "tags": [
      "lightweight",
      "flagship",
      "ai"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-16/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 4799,
        "pricePdd": 4459,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 7499,
        "pricePdd": 6949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.1英寸",
      "resolution": "2556×1179",
      "refreshRate": "60Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "147.6mm",
      "width": "71.6mm",
      "thickness": "7.8mm",
      "weight": "170g",
      "material": "航空级铝金属"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-16-plus",
    "brand": "苹果",
    "model": "iPhone 16 Plus",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2016%20Plus%20Black.jpg",
    "launchDate": "2024-09",
    "tags": [
      "大屏",
      "长续航",
      "ai"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-16/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 5799,
        "pricePdd": 5359,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 8499,
        "pricePdd": 7899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2796×1290",
      "refreshRate": "60Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160.9mm",
      "width": "77.8mm",
      "thickness": "7.8mm",
      "weight": "199g",
      "material": "航空级铝金属"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-16-pro",
    "brand": "苹果",
    "model": "iPhone 16 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2016%20Pro%20Black.jpg",
    "launchDate": "2024-09",
    "tags": [
      "flagship",
      "专业影像",
      "ai"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-16-pro/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 6799,
        "pricePdd": 6299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 7999,
        "pricePdd": 7449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 9499,
        "pricePdd": 8799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 10999,
        "pricePdd": 10249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.3英寸",
      "resolution": "2622×1206",
      "refreshRate": "120Hz",
      "material": "OLED (ProMotion)",
      "type": "直屏"
    },
    "body": {
      "height": "149.6mm",
      "width": "71.5mm",
      "thickness": "8.25mm",
      "weight": "199g",
      "material": "钛金属"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-16-pro-max",
    "brand": "苹果",
    "model": "iPhone 16 Pro Max",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2016%20Pro%20Max%20Black.jpg",
    "launchDate": "2024-09",
    "tags": [
      "超大屏",
      "最强续航",
      "专业影像"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-16-pro/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 8999,
        "pricePdd": 8399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 10499,
        "pricePdd": 9749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 11999,
        "pricePdd": 11249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "2868×1320",
      "refreshRate": "120Hz",
      "material": "OLED (ProMotion)",
      "type": "直屏"
    },
    "body": {
      "height": "163mm",
      "width": "77.6mm",
      "thickness": "8.25mm",
      "weight": "227g",
      "material": "钛金属"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-15",
    "brand": "苹果",
    "model": "iPhone 15",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2015%20Black.jpg",
    "launchDate": "2023-09",
    "tags": [
      "value",
      "lightweight",
      "usb_c"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-15/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 4099,
        "pricePdd": 3799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.1英寸",
      "resolution": "2556×1179",
      "refreshRate": "60Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "147.6mm",
      "width": "71.6mm",
      "thickness": "7.8mm",
      "weight": "171g",
      "material": "航空级铝金属"
    },
    "ram": "6GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-14",
    "brand": "苹果",
    "model": "iPhone 14",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2014%20Black.jpg",
    "launchDate": "2022-09",
    "tags": [
      "入门",
      "lightweight",
      "降价"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-14/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 3599,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 4399,
        "pricePdd": 3999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5899,
        "pricePdd": 5449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.1英寸",
      "resolution": "2532×1170",
      "refreshRate": "60Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "146.7mm",
      "width": "71.5mm",
      "thickness": "7.8mm",
      "weight": "172g",
      "material": "航空级铝金属"
    },
    "ram": "6GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "Lightning",
      "waterproof": "IP68"
    }
  },
  {
    "id": "mate-70",
    "brand": "华为",
    "model": "Mate 70",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Mate%2060%20Pro.jpg",
    "launchDate": "2024-11",
    "tags": [
      "商务旗舰",
      "鸿蒙",
      "卫星通信"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/mate70/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2688×1216",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "160.9mm",
      "width": "75.9mm",
      "thickness": "7.8mm",
      "weight": "203g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "mate-70-pro",
    "brand": "华为",
    "model": "Mate 70 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Mate%2060%20Pro%20Plus.jpg",
    "launchDate": "2024-11",
    "tags": [
      "旗舰影像",
      "3d人脸",
      "卫星通信"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/mate70/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 6049,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7999,
        "pricePdd": 7449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "2832×1316",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "164.6mm",
      "width": "79.5mm",
      "thickness": "8.1mm",
      "weight": "221g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "mate-70-pro-plus",
    "brand": "华为",
    "model": "Mate 70 Pro+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Mate%2060%20Pro%20Plus.jpg",
    "launchDate": "2024-11",
    "tags": [
      "超旗舰",
      "钛合金",
      "最强影像"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/mate70/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 8499,
        "pricePdd": 7899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 9499,
        "pricePdd": 8849,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "2832×1316",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "164.6mm",
      "width": "79.5mm",
      "thickness": "8.1mm",
      "weight": "226g",
      "material": "钛合金+锦纤"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "mate-60",
    "brand": "华为",
    "model": "Mate 60",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Mate%2060%20Pro.jpg",
    "launchDate": "2023-08",
    "tags": [
      "经典旗舰",
      "降价",
      "卫星通信"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/mate60/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.69英寸",
      "resolution": "2688×1216",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "161.4mm",
      "width": "76mm",
      "thickness": "7.95mm",
      "weight": "209g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-80",
    "brand": "华为",
    "model": "Pura 80",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080.jpg",
    "launchDate": "2025-04",
    "tags": [
      "影像旗舰",
      "时尚设计",
      "鸿蒙"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura80/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.6英寸",
      "resolution": "2760×1256",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "157.6mm",
      "width": "74.3mm",
      "thickness": "7.95mm",
      "weight": "197g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-80-pro",
    "brand": "华为",
    "model": "Pura 80 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080%20Pro.jpg",
    "launchDate": "2025-04",
    "tags": [
      "专业影像",
      "长焦微距",
      "时尚设计"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura80/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7999,
        "pricePdd": 7449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2844×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "215g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-80-ultra",
    "brand": "华为",
    "model": "Pura 80 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080%20Ultra.jpg",
    "launchDate": "2025-04",
    "tags": [
      "影像机皇",
      "1英寸大底",
      "伸缩镜头"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura80/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 9999,
        "pricePdd": 9299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 10999,
        "pricePdd": 10249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2844×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "221g",
      "material": "素皮"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "nova-14-pro",
    "brand": "华为",
    "model": "nova 14 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20nova%2012%20Pro.jpg",
    "launchDate": "2025-05",
    "tags": [
      "自拍",
      "lightweight",
      "时尚"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/nova14-pro/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3649,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2760×1236",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "163.3mm",
      "width": "74.7mm",
      "thickness": "7.68mm",
      "weight": "188g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "nova-14",
    "brand": "华为",
    "model": "nova 14",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20nova%2012.jpg",
    "launchDate": "2025-05",
    "tags": [
      "value",
      "lightweight",
      "自拍"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/nova14/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3199,
        "pricePdd": 2899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2412×1084",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "161.8mm",
      "width": "74.5mm",
      "thickness": "6.98mm",
      "weight": "185g",
      "material": "塑料中框+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "生活防水"
    }
  },
  {
    "id": "enjoy-70x",
    "brand": "华为",
    "model": "畅享 70X",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Enjoy%2070.jpg",
    "launchDate": "2025-01",
    "tags": [
      "千元机",
      "长续航",
      "鸿蒙"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/enjoy-70x/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 1599,
        "pricePdd": 1449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.75英寸",
      "resolution": "2600×1200",
      "refreshRate": "90Hz",
      "material": "LCD",
      "type": "直屏"
    },
    "body": {
      "height": "168.3mm",
      "width": "77.7mm",
      "thickness": "8.93mm",
      "weight": "207g",
      "material": "塑料"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "不支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 5",
      "bluetooth": "5.1",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "生活防水"
    }
  },
  {
    "id": "xiaomi-16",
    "brand": "小米",
    "model": "Xiaomi 16",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2015.jpg",
    "launchDate": "2025-10",
    "tags": [
      "小屏旗舰",
      "徕卡影像",
      "lightweight"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-16",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4299,
        "pricePdd": 3949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4799,
        "pricePdd": 4399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5499,
        "pricePdd": 5049,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.36英寸",
      "resolution": "2670×1200",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "152.8mm",
      "width": "71.5mm",
      "thickness": "8.2mm",
      "weight": "187g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-16-pro",
    "brand": "小米",
    "model": "Xiaomi 16 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2015%20Ultra.jpg",
    "launchDate": "2025-10",
    "tags": [
      "全能旗舰",
      "徕卡影像",
      "大屏"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-16-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5299,
        "pricePdd": 4899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5799,
        "pricePdd": 5349,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.73英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "161.3mm",
      "width": "75.3mm",
      "thickness": "8.5mm",
      "weight": "213g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-15",
    "brand": "小米",
    "model": "Xiaomi 15",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2015.jpg",
    "launchDate": "2024-10",
    "tags": [
      "小屏旗舰",
      "徕卡影像",
      "降价"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-15",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3649,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4699,
        "pricePdd": 4349,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.36英寸",
      "resolution": "2670×1200",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "152.8mm",
      "width": "71.5mm",
      "thickness": "8.2mm",
      "weight": "188g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-15-ultra",
    "brand": "小米",
    "model": "Xiaomi 15 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2015%20Ultra.jpg",
    "launchDate": "2025-02",
    "tags": [
      "影像机皇",
      "1英寸大底",
      "徕卡"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-15-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7799,
        "pricePdd": 7249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.73英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "161.3mm",
      "width": "75.3mm",
      "thickness": "9.2mm",
      "weight": "229g",
      "material": "铝合金+素皮"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "redmi-k80-pro",
    "brand": "小米",
    "model": "Redmi K80 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Redmi%20K70%20Pro%2023117RK66C%20Black.jpg",
    "launchDate": "2024-11",
    "tags": [
      "value",
      "高性能",
      "大电池"
    ],
    "officialUrl": "https://www.mi.com/redmi-k80-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3299,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3699,
        "pricePdd": 3399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4399,
        "pricePdd": 4049,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160mm",
      "width": "75mm",
      "thickness": "8.1mm",
      "weight": "206g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "redmi-k80",
    "brand": "小米",
    "model": "Redmi K80",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Redmi%20K70%2023113RKC6C%20Black.jpg",
    "launchDate": "2024-11",
    "tags": [
      "value",
      "大电池",
      "水桶机"
    ],
    "officialUrl": "https://www.mi.com/redmi-k80",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2499,
        "pricePdd": 2249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2899,
        "pricePdd": 2649,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160mm",
      "width": "75mm",
      "thickness": "8.1mm",
      "weight": "201g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "redmi-note-14-pro-plus",
    "brand": "小米",
    "model": "Redmi Note 14 Pro+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Redmi%20Note%2014%20Pro%20Plus%205G.jpg",
    "launchDate": "2025-03",
    "tags": [
      "千元旗舰",
      "大电量",
      "快充"
    ],
    "officialUrl": "https://www.mi.com/redmi-note-14-pro-plus",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2299,
        "pricePdd": 2099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2712×1220",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "162.5mm",
      "width": "74.5mm",
      "thickness": "8.7mm",
      "weight": "195g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "redmi-note-14-pro",
    "brand": "小米",
    "model": "Redmi Note 14 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Redmi%20Note%2014%20Pro.jpg",
    "launchDate": "2025-03",
    "tags": [
      "千元机",
      "长续航",
      "护眼屏"
    ],
    "officialUrl": "https://www.mi.com/redmi-note-14-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 1499,
        "pricePdd": 1349,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 1699,
        "pricePdd": 1549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2400×1080",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "162.5mm",
      "width": "74.5mm",
      "thickness": "7.98mm",
      "weight": "187g",
      "material": "塑料"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "redmi-turbo-4",
    "brand": "小米",
    "model": "Redmi Turbo 4",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Redmi%20Note%2013%20Pro.jpg",
    "launchDate": "2025-01",
    "tags": [
      "性能",
      "gaming",
      "value"
    ],
    "officialUrl": "https://www.mi.com/redmi-turbo-4",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2299,
        "pricePdd": 2099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3199,
        "pricePdd": 2899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2712×1220",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160.8mm",
      "width": "74.3mm",
      "thickness": "8.05mm",
      "weight": "198g",
      "material": "塑料"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP64"
    }
  },
  {
    "id": "iphone-16e",
    "brand": "苹果",
    "model": "iPhone 16e",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/ios/iPhone%2016e%20Black.jpg",
    "launchDate": "2025-02",
    "tags": [
      "value",
      "lightweight",
      "a18芯片"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-16e/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.1英寸",
      "resolution": "2532×1170",
      "refreshRate": "60Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "146.7mm",
      "width": "71.5mm",
      "thickness": "7.8mm",
      "weight": "167g",
      "material": "航空级铝金属"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "mate-70-rs",
    "brand": "华为",
    "model": "Mate 70 RS 非凡大师",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Mate%2060%20RS%20Ultimate.jpg",
    "launchDate": "2024-11",
    "tags": [
      "超旗舰",
      "陶瓷机身",
      "卫星通信"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/mate70-rs/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 11999,
        "pricePdd": 11299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 12999,
        "pricePdd": 12249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "2832×1316",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "164.6mm",
      "width": "79.5mm",
      "thickness": "8.1mm",
      "weight": "242g",
      "material": "陶瓷"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-70",
    "brand": "华为",
    "model": "Pura 70",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080.jpg",
    "launchDate": "2024-04",
    "tags": [
      "影像旗舰",
      "降价",
      "harmonyos"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura70/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.6英寸",
      "resolution": "2760×1256",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "157.6mm",
      "width": "74.3mm",
      "thickness": "7.95mm",
      "weight": "207g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-70-pro",
    "brand": "华为",
    "model": "Pura 70 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080%20Pro.jpg",
    "launchDate": "2024-04",
    "tags": [
      "专业影像",
      "长焦微距",
      "降价"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura70-pro/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2844×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "220g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-70-pro-plus",
    "brand": "华为",
    "model": "Pura 70 Pro+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080%20Pro.jpg",
    "launchDate": "2024-04",
    "tags": [
      "旗舰影像",
      "玄武钢化",
      "降价"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura70-pro-plus/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7999,
        "pricePdd": 7449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2844×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "220g",
      "material": "铝合金+玻璃"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "pura-70-ultra",
    "brand": "华为",
    "model": "Pura 70 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20Pura%2080%20Ultra.jpg",
    "launchDate": "2024-04",
    "tags": [
      "影像机皇",
      "1英寸大底",
      "伸缩镜头"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/pura70-ultra/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 8999,
        "pricePdd": 8399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 9999,
        "pricePdd": 9299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2844×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "226g",
      "material": "素皮"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "nova-13",
    "brand": "华为",
    "model": "nova 13",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20nova%2012.jpg",
    "launchDate": "2024-10",
    "tags": [
      "中端",
      "lightweight",
      "自拍"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/nova13/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2399,
        "pricePdd": 2199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2899,
        "pricePdd": 2649,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2412×1084",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "161.8mm",
      "width": "74.5mm",
      "thickness": "6.98mm",
      "weight": "185g",
      "material": "塑料中框+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "生活防水"
    }
  },
  {
    "id": "nova-13-pro",
    "brand": "华为",
    "model": "nova 13 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Huawei%20nova%2012%20Pro.jpg",
    "launchDate": "2024-10",
    "tags": [
      "自拍旗舰",
      "lightweight",
      "曲屏"
    ],
    "officialUrl": "https://consumer.huawei.com/cn/phones/nova13-pro/",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3199,
        "pricePdd": 2949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3699,
        "pricePdd": 3399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2760×1236",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "163.3mm",
      "width": "74.7mm",
      "thickness": "7.68mm",
      "weight": "188g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.2",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "xiaomi-15-pro",
    "brand": "小米",
    "model": "Xiaomi 15 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2015%20Ultra.jpg",
    "launchDate": "2024-10",
    "tags": [
      "全能旗舰",
      "2k屏",
      "徕卡长焦"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-15-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4799,
        "pricePdd": 4449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5299,
        "pricePdd": 4899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.73英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "161.3mm",
      "width": "75.3mm",
      "thickness": "8.5mm",
      "weight": "213g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-14-ultra",
    "brand": "小米",
    "model": "Xiaomi 14 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%2014%20Ultra.jpg",
    "launchDate": "2024-02",
    "tags": [
      "影像机皇",
      "1英寸大底",
      "徕卡全焦段"
    ],
    "officialUrl": "https://www.mi.com/xiaomi-14-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6799,
        "pricePdd": 6299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.73英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "161.4mm",
      "width": "75.3mm",
      "thickness": "9.2mm",
      "weight": "224g",
      "material": "铝合金+素皮"
    },
    "ram": "16GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-civi-4-pro",
    "brand": "小米",
    "model": "Xiaomi Civi 4 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Civi%203.jpg",
    "launchDate": "2024-03",
    "tags": [
      "自拍",
      "lightweight",
      "时尚设计"
    ],
    "officialUrl": "https://www.mi.com/civi-4-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3199,
        "pricePdd": 2899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.55英寸",
      "resolution": "2750×1236",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "157.2mm",
      "width": "72.8mm",
      "thickness": "7.45mm",
      "weight": "179g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "redmi-note-14",
    "brand": "小米",
    "model": "Redmi Note 14",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Redmi%20Note%2014%205G.jpg",
    "launchDate": "2025-03",
    "tags": [
      "千元机",
      "长续航",
      "value"
    ],
    "officialUrl": "https://www.mi.com/redmi-note-14",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 1199,
        "pricePdd": 1099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": 1399,
        "pricePdd": 1249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 1699,
        "pricePdd": 1549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2400×1080",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "162.5mm",
      "width": "74.5mm",
      "thickness": "7.98mm",
      "weight": "190g",
      "material": "塑料"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "redmi-k70-ultra",
    "brand": "小米",
    "model": "Redmi K70 至尊版",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Redmi%20K70%2023113RKC6C%20Black.jpg",
    "launchDate": "2024-07",
    "tags": [
      "性能旗舰",
      "天玑9300"
    ],
    "officialUrl": "https://www.mi.com/redmi-k70-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2599,
        "pricePdd": 2399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2999,
        "pricePdd": 2749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3599,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2712×1220",
      "refreshRate": "144Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160.4mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "211g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "redmi-turbo-3",
    "brand": "小米",
    "model": "Redmi Turbo 3",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Xiaomi%20Redmi%20Note%2013%20Pro.jpg",
    "launchDate": "2024-04",
    "tags": [
      "性能",
      "lightweight",
      "骁龙8s"
    ],
    "officialUrl": "https://www.mi.com/redmi-turbo-3",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2299,
        "pricePdd": 2099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 2799,
        "pricePdd": 2549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2712×1220",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160.5mm",
      "width": "74.4mm",
      "thickness": "7.8mm",
      "weight": "179g",
      "material": "塑料"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP64"
    }
  },
  {
    "id": "vivo-x200-pro-mini",
    "brand": "vivo",
    "model": "X200 Pro mini",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200%20Pro%20mini%20Titanium.jpg",
    "launchDate": "2024-10",
    "tags": [
      "小屏旗舰",
      "蔡司影像",
      "天玑9400"
    ],
    "officialUrl": "https://www.vivo.com.cn/x200-pro-mini",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4299,
        "pricePdd": 3949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4799,
        "pricePdd": 4399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5499,
        "pricePdd": 5049,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.31英寸",
      "resolution": "2640×1216",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "150.8mm",
      "width": "71.8mm",
      "thickness": "8.15mm",
      "weight": "187g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-x200",
    "brand": "vivo",
    "model": "X200",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200%20Black.jpg",
    "launchDate": "2024-10",
    "tags": [
      "flagship",
      "蔡司长焦",
      "天玑9400"
    ],
    "officialUrl": "https://www.vivo.com.cn/x200",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5199,
        "pricePdd": 4799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "160.3mm",
      "width": "74.8mm",
      "thickness": "8mm",
      "weight": "197g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "vivo-x200-pro",
    "brand": "vivo",
    "model": "X200 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200%20Pro%20Black.jpg",
    "launchDate": "2024-10",
    "tags": [
      "影像旗舰",
      "蔡司apo",
      "2亿长焦"
    ],
    "officialUrl": "https://www.vivo.com.cn/x200-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6199,
        "pricePdd": 5749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "162.4mm",
      "width": "76mm",
      "thickness": "8.5mm",
      "weight": "223g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-x100-ultra",
    "brand": "vivo",
    "model": "X100 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200%20Pro.jpg",
    "launchDate": "2024-05",
    "tags": [
      "影像机皇",
      "2亿apo长焦",
      "1英寸"
    ],
    "officialUrl": "https://www.vivo.com.cn/x100-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6799,
        "pricePdd": 6299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "3200×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "164.1mm",
      "width": "75.6mm",
      "thickness": "9.2mm",
      "weight": "229g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-s20-pro",
    "brand": "vivo",
    "model": "S20 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20S20%20Pro.jpg",
    "launchDate": "2024-12",
    "tags": [
      "人像",
      "lightweight",
      "自拍"
    ],
    "officialUrl": "https://www.vivo.com.cn/s20-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2999,
        "pricePdd": 2749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "164.4mm",
      "width": "75.1mm",
      "thickness": "7.5mm",
      "weight": "185g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP64"
    }
  },
  {
    "id": "iqoo-13",
    "brand": "vivo",
    "model": "iQOO 13",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20iQOO%2013.jpg",
    "launchDate": "2024-11",
    "tags": [
      "gaming",
      "骁龙8elite"
    ],
    "officialUrl": "https://www.vivo.com.cn/iqoo-13",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3699,
        "pricePdd": 3399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4199,
        "pricePdd": 3849,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4899,
        "pricePdd": 4499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸",
      "resolution": "3168×1440",
      "refreshRate": "144Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "163.2mm",
      "width": "76.8mm",
      "thickness": "8mm",
      "weight": "207g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iqoo-neo10-pro",
    "brand": "vivo",
    "model": "iQOO Neo10 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20iQOO%20Neo%2010%20Pro.jpg",
    "launchDate": "2024-12",
    "tags": [
      "gaming",
      "天玑9400"
    ],
    "officialUrl": "https://www.vivo.com.cn/iqoo-neo10-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3099,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3699,
        "pricePdd": 3349,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "144Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "163.5mm",
      "width": "76.8mm",
      "thickness": "8.1mm",
      "weight": "199g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP54"
    }
  },
  {
    "id": "oppo-find-x8",
    "brand": "OPPO",
    "model": "Find X8",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Find%20X8%20Pro.jpg",
    "launchDate": "2024-10",
    "tags": [
      "flagship",
      "哈苏影像",
      "天玑9400"
    ],
    "officialUrl": "https://www.oppo.com/cn/find-x8",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.59英寸",
      "resolution": "2760×1256",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "157.4mm",
      "width": "74.3mm",
      "thickness": "7.85mm",
      "weight": "193g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oppo-find-x8-pro",
    "brand": "OPPO",
    "model": "Find X8 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Find%20X8%20Pro.jpg",
    "launchDate": "2024-10",
    "tags": [
      "旗舰影像",
      "双潜望长焦",
      "哈苏"
    ],
    "officialUrl": "https://www.oppo.com/cn/find-x8-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "162.3mm",
      "width": "76.6mm",
      "thickness": "8.24mm",
      "weight": "215g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oppo-find-x7-ultra",
    "brand": "OPPO",
    "model": "Find X7 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Find%20X7%20Ultra.jpg",
    "launchDate": "2024-01",
    "tags": [
      "影像机皇",
      "双潜望",
      "1英寸"
    ],
    "officialUrl": "https://www.oppo.com/cn/find-x7-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸",
      "resolution": "3168×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "164.3mm",
      "width": "76.2mm",
      "thickness": "9.5mm",
      "weight": "221g",
      "material": "铝合金+素皮/玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "oppo-find-x7",
    "brand": "OPPO",
    "model": "Find X7",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Find%20X7.jpg",
    "launchDate": "2024-01",
    "tags": [
      "flagship",
      "哈苏",
      "天玑9300"
    ],
    "officialUrl": "https://www.oppo.com/cn/find-x7",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3299,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3799,
        "pricePdd": 3449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4299,
        "pricePdd": 3949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "162.7mm",
      "width": "75.4mm",
      "thickness": "8.7mm",
      "weight": "202g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "oppo-reno13-pro",
    "brand": "OPPO",
    "model": "Reno 13 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Reno13%20Pro.jpg",
    "launchDate": "2024-11",
    "tags": [
      "人像",
      "lightweight",
      "coloros"
    ],
    "officialUrl": "https://www.oppo.com/cn/reno13-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3299,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3799,
        "pricePdd": 3449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.83英寸",
      "resolution": "2800×1272",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "162.8mm",
      "width": "76.6mm",
      "thickness": "7.55mm",
      "weight": "192g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oppo-reno13",
    "brand": "OPPO",
    "model": "Reno 13",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Reno13.jpg",
    "launchDate": "2024-11",
    "tags": [
      "lightweight",
      "自拍",
      "长续航"
    ],
    "officialUrl": "https://www.oppo.com/cn/reno13",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2599,
        "pricePdd": 2399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3099,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.59英寸",
      "resolution": "2760×1256",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "157.9mm",
      "width": "74.7mm",
      "thickness": "7.24mm",
      "weight": "181g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "honor-magic7",
    "brand": "荣耀",
    "model": "Magic7",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20Magic7%20Pro.jpg",
    "launchDate": "2024-11",
    "tags": [
      "ai旗舰",
      "骁龙8elite",
      "荣耀绿洲护眼"
    ],
    "officialUrl": "https://www.honor.com/cn/magic7",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5699,
        "pricePdd": 5249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1264",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "162.1mm",
      "width": "75.8mm",
      "thickness": "7.95mm",
      "weight": "199g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "honor-magic7-pro",
    "brand": "荣耀",
    "model": "Magic7 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20Magic7%20Pro.jpg",
    "launchDate": "2024-11",
    "tags": [
      "ai旗舰",
      "3d人脸",
      "2亿长焦"
    ],
    "officialUrl": "https://www.honor.com/cn/magic7-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 5999,
        "pricePdd": 5549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 6699,
        "pricePdd": 6199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2800×1280",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "等深微曲屏"
    },
    "body": {
      "height": "162.7mm",
      "width": "77.1mm",
      "thickness": "8.5mm",
      "weight": "223g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "honor-magic6-pro",
    "brand": "荣耀",
    "model": "Magic6 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20Magic6%20Pro.jpg",
    "launchDate": "2024-01",
    "tags": [
      "flagship",
      "1_8亿长焦",
      "降价"
    ],
    "officialUrl": "https://www.honor.com/cn/magic6-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4299,
        "pricePdd": 3949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4799,
        "pricePdd": 4399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5499,
        "pricePdd": 5049,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "2800×1280",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "164.2mm",
      "width": "76.5mm",
      "thickness": "8.9mm",
      "weight": "225g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "honor-400-pro",
    "brand": "荣耀",
    "model": "400 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20400%20Pro.jpg",
    "launchDate": "2025-05",
    "tags": [
      "中端旗舰",
      "绿洲护眼",
      "ai"
    ],
    "officialUrl": "https://www.honor.com/cn/400-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3299,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3799,
        "pricePdd": 3449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2700×1224",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "163.5mm",
      "width": "75.5mm",
      "thickness": "7.8mm",
      "weight": "188g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "honor-300-pro",
    "brand": "荣耀",
    "model": "300 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20300%20Pro.jpg",
    "launchDate": "2024-12",
    "tags": [
      "中端",
      "绿洲护眼",
      "lightweight"
    ],
    "officialUrl": "https://www.honor.com/cn/300-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2799,
        "pricePdd": 2549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3299,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2700×1224",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "163.5mm",
      "width": "75.5mm",
      "thickness": "7.8mm",
      "weight": "187g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "samsung-s25",
    "brand": "三星",
    "model": "Galaxy S25",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S25.jpg",
    "launchDate": "2025-01",
    "tags": [
      "小屏旗舰",
      "骁龙8elite",
      "one_ui"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s25",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.2英寸",
      "resolution": "2340×1080",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "146.9mm",
      "width": "70.5mm",
      "thickness": "7.2mm",
      "weight": "162g",
      "material": "装甲铝+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s25-plus",
    "brand": "三星",
    "model": "Galaxy S25+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S25%20Plus.jpg",
    "launchDate": "2025-01",
    "tags": [
      "大屏旗舰",
      "2k屏",
      "ai"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s25-plus",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 7499,
        "pricePdd": 6949,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "3120×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "158.4mm",
      "width": "75.8mm",
      "thickness": "7.3mm",
      "weight": "190g",
      "material": "装甲铝+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s25-ultra",
    "brand": "三星",
    "model": "Galaxy S25 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S25%20Ultra.jpg",
    "launchDate": "2025-01",
    "tags": [
      "安卓机皇",
      "s_pen",
      "2亿像素"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s25-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 8499,
        "pricePdd": 7899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 9499,
        "pricePdd": 8849,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 10999,
        "pricePdd": 10249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "3120×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "162.8mm",
      "width": "77.6mm",
      "thickness": "8.2mm",
      "weight": "218g",
      "material": "钛金属+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s24",
    "brand": "三星",
    "model": "Galaxy S24",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S24.jpg",
    "launchDate": "2024-01",
    "tags": [
      "小屏旗舰",
      "ai",
      "降价"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s24",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.2英寸",
      "resolution": "2340×1080",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "147mm",
      "width": "70.6mm",
      "thickness": "7.6mm",
      "weight": "167g",
      "material": "装甲铝+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6E",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s24-ultra",
    "brand": "三星",
    "model": "Galaxy S24 Ultra",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S24%20Ultra.jpg",
    "launchDate": "2024-01",
    "tags": [
      "安卓机皇",
      "s_pen",
      "降价"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s24-ultra",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6999,
        "pricePdd": 6499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 7999,
        "pricePdd": 7449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 8999,
        "pricePdd": 8399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸",
      "resolution": "3120×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "162.3mm",
      "width": "79mm",
      "thickness": "8.6mm",
      "weight": "232g",
      "material": "钛金属+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "vivo-x100s",
    "brand": "vivo",
    "model": "X100s",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200.jpg",
    "launchDate": "2024-05",
    "tags": [
      "直屏旗舰",
      "天玑9300",
      "蔡司"
    ],
    "officialUrl": "https://www.vivo.com.cn/x100s",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3599,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4099,
        "pricePdd": 3749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4699,
        "pricePdd": 4349,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "163.1mm",
      "width": "75.8mm",
      "thickness": "8mm",
      "weight": "203g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-x100s-pro",
    "brand": "vivo",
    "model": "X100s Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20X200%20Pro.jpg",
    "launchDate": "2024-05",
    "tags": [
      "影像旗舰",
      "1英寸",
      "天玑9300"
    ],
    "officialUrl": "https://www.vivo.com.cn/x100s-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4499,
        "pricePdd": 4149,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": 4599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5699,
        "pricePdd": 5249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "164.1mm",
      "width": "75.3mm",
      "thickness": "8.9mm",
      "weight": "225g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "iqoo-z9-turbo-plus",
    "brand": "vivo",
    "model": "iQOO Z9 Turbo+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/vivo%20iQOO%20Z9.jpg",
    "launchDate": "2024-09",
    "tags": [
      "gaming",
      "天玑9300",
      "大电池"
    ],
    "officialUrl": "https://www.vivo.com.cn/iqoo-z9-turbo-plus",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2199,
        "pricePdd": 1999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2599,
        "pricePdd": 2399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1260",
      "refreshRate": "144Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "163.8mm",
      "width": "76mm",
      "thickness": "8mm",
      "weight": "196g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP64"
    }
  },
  {
    "id": "oppo-reno12-pro",
    "brand": "OPPO",
    "model": "Reno 12 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Reno12%20Pro.jpg",
    "launchDate": "2024-05",
    "tags": [
      "lightweight",
      "人像",
      "ai"
    ],
    "officialUrl": "https://www.oppo.com/cn/reno12-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3199,
        "pricePdd": 2899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2412×1080",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "161.4mm",
      "width": "74.1mm",
      "thickness": "7.6mm",
      "weight": "184g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "oppo-reno12",
    "brand": "OPPO",
    "model": "Reno 12",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Oppo%20Reno12.jpg",
    "launchDate": "2024-05",
    "tags": [
      "lightweight",
      "自拍",
      "coloros"
    ],
    "officialUrl": "https://www.oppo.com/cn/reno12",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2499,
        "pricePdd": 2249,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2412×1080",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "161.4mm",
      "width": "74.1mm",
      "thickness": "7.3mm",
      "weight": "179g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "honor-magic6",
    "brand": "荣耀",
    "model": "Magic6",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20Magic6.jpg",
    "launchDate": "2024-01",
    "tags": [
      "flagship",
      "骁龙8gen3",
      "降价"
    ],
    "officialUrl": "https://www.honor.com/cn/magic6",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3649,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1264",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "曲屏"
    },
    "body": {
      "height": "161.8mm",
      "width": "75.4mm",
      "thickness": "8.1mm",
      "weight": "206g",
      "material": "铝合金+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3.1)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "honor-400",
    "brand": "荣耀",
    "model": "400",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20400.jpg",
    "launchDate": "2025-05",
    "tags": [
      "中端",
      "绿洲护眼",
      "lightweight"
    ],
    "officialUrl": "https://www.honor.com/cn/400",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2699,
        "pricePdd": 2449,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3199,
        "pricePdd": 2899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2664×1200",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "161.5mm",
      "width": "74.6mm",
      "thickness": "7.7mm",
      "weight": "181g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "honor-300",
    "brand": "荣耀",
    "model": "300",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20300.jpg",
    "launchDate": "2024-12",
    "tags": [
      "中端",
      "护眼",
      "value"
    ],
    "officialUrl": "https://www.honor.com/cn/300",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2299,
        "pricePdd": 2099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2799,
        "pricePdd": 2549,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2664×1200",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "161.5mm",
      "width": "74.6mm",
      "thickness": "7.7mm",
      "weight": "179g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "honor-200-pro",
    "brand": "荣耀",
    "model": "200 Pro",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Honor%20200%20Pro.jpg",
    "launchDate": "2024-05",
    "tags": [
      "人像",
      "雅顾光影",
      "骁龙8s"
    ],
    "officialUrl": "https://www.honor.com/cn/200-pro",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2999,
        "pricePdd": 2749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3499,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2700×1224",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "曲屏"
    },
    "body": {
      "height": "163.5mm",
      "width": "75.5mm",
      "thickness": "8.2mm",
      "weight": "199g",
      "material": "塑料中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "samsung-s24-plus",
    "brand": "三星",
    "model": "Galaxy S24+",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20S24%20Plus%20Amber%20Yellow.jpg",
    "launchDate": "2024-01",
    "tags": [
      "大屏旗舰",
      "2k屏",
      "ai"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-s24-plus",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5499,
        "pricePdd": 5099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6499,
        "pricePdd": 5999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "3120×1440",
      "refreshRate": "120Hz",
      "material": "OLED (LTPO)",
      "type": "直屏"
    },
    "body": {
      "height": "158.5mm",
      "width": "75.9mm",
      "thickness": "7.7mm",
      "weight": "196g",
      "material": "装甲铝+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6E",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-a56",
    "brand": "三星",
    "model": "Galaxy A56 5G",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20A56%205G.jpg",
    "launchDate": "2025-03",
    "tags": [
      "中端",
      "amoled",
      "one_ui"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-a56",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2999,
        "pricePdd": 2749,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.7英寸",
      "resolution": "2340×1080",
      "refreshRate": "120Hz",
      "material": "Super AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "162.2mm",
      "width": "77.5mm",
      "thickness": "8.2mm",
      "weight": "198g",
      "material": "塑料中框+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP67"
    }
  },
  {
    "id": "samsung-a55",
    "brand": "三星",
    "model": "Galaxy A55 5G",
    "image": "https://cdn.jsdelivr.net/gh/orcastor/phone_images@master/android/Samsung%20Galaxy%20A55.jpg",
    "launchDate": "2024-03",
    "tags": [
      "中端",
      "金属中框",
      "降价"
    ],
    "officialUrl": "https://www.samsung.com/cn/galaxy-a55",
    "lastVerified": "2026-07-15",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2399,
        "pricePdd": 2199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.6英寸",
      "resolution": "2340×1080",
      "refreshRate": "120Hz",
      "material": "Super AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "161.1mm",
      "width": "77.4mm",
      "thickness": "8.2mm",
      "weight": "213g",
      "material": "铝合金+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP67"
    }
  },
  {
    "id": "oneplus-13",
    "brand": "一加",
    "model": "OnePlus 13",
    "image": "",
    "launchDate": "2024-12",
    "tags": [
      "flagship",
      "2k屏",
      "长续航"
    ],
    "officialUrl": "https://www.oneplus.com/cn/oneplus-13",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4499,
        "pricePdd": 4199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5999,
        "pricePdd": 5699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸",
      "resolution": "3168×1440",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "全等深微曲屏"
    },
    "body": {
      "height": "162.9mm",
      "width": "76.5mm",
      "thickness": "8.5mm",
      "weight": "210g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oneplus-ace5",
    "brand": "一加",
    "model": "OnePlus Ace 5",
    "image": "",
    "launchDate": "2024-12",
    "tags": [
      "value",
      "gaming",
      "长续航"
    ],
    "officialUrl": "https://www.oneplus.com/cn/oneplus-ace-5",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2499,
        "pricePdd": 2299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2999,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3499,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "161.7mm",
      "width": "75.3mm",
      "thickness": "8mm",
      "weight": "206g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "oneplus-ace5-pro",
    "brand": "一加",
    "model": "OnePlus Ace 5 Pro",
    "image": "",
    "launchDate": "2024-12",
    "tags": [
      "旗舰芯",
      "gaming",
      "陶瓷"
    ],
    "officialUrl": "https://www.oneplus.com/cn/oneplus-ace-5-pro",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3399,
        "pricePdd": 3199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4699,
        "pricePdd": 4399,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "161.7mm",
      "width": "75.3mm",
      "thickness": "8.1mm",
      "weight": "203g",
      "material": "金属中框+陶瓷"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "oneplus-13t",
    "brand": "一加",
    "model": "OnePlus 13T",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "小屏旗舰",
      "长续航",
      "旗舰芯"
    ],
    "officialUrl": "https://www.oneplus.com/cn/oneplus-13t",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4499,
        "pricePdd": 4199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.32英寸",
      "resolution": "2640×1216",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "150.8mm",
      "width": "72mm",
      "thickness": "8.1mm",
      "weight": "187g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "realme-gt7-pro",
    "brand": "真我",
    "model": "realme GT7 Pro",
    "image": "",
    "launchDate": "2024-11",
    "tags": [
      "flagship",
      "长续航",
      "潜望长焦"
    ],
    "officialUrl": "https://www.realme.com/cn/realme-gt7-pro",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": 3299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3999,
        "pricePdd": 3699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4499,
        "pricePdd": 4199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "162.5mm",
      "width": "76.9mm",
      "thickness": "8.6mm",
      "weight": "222g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "realme-gt6",
    "brand": "真我",
    "model": "realme GT6",
    "image": "",
    "launchDate": "2024-07",
    "tags": [
      "value",
      "电竞屏",
      "快充"
    ],
    "officialUrl": "https://www.realme.com/cn/realme-gt6",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2099,
        "pricePdd": 1899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2599,
        "pricePdd": 2299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3099,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.74英寸",
      "resolution": "2772×1240",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "162mm",
      "width": "75.1mm",
      "thickness": "8.4mm",
      "weight": "199g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 6",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP65"
    }
  },
  {
    "id": "realme-neo7",
    "brand": "真我",
    "model": "realme Neo7",
    "image": "",
    "launchDate": "2024-12",
    "tags": [
      "超大电池",
      "value",
      "gaming"
    ],
    "officialUrl": "https://www.realme.com/cn/realme-neo7",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2099,
        "pricePdd": 1899,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2499,
        "pricePdd": 2299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3099,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2780×1264",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "162.6mm",
      "width": "76.7mm",
      "thickness": "8.6mm",
      "weight": "213g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oppo-find-x8-ultra",
    "brand": "OPPO",
    "model": "OPPO Find X8 Ultra",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "影像旗舰",
      "2k屏",
      "长焦"
    ],
    "officialUrl": "https://www.oppo.com/cn/smartphones/series-find-x/find-x8-ultra/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 6199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7999,
        "pricePdd": 7699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸",
      "resolution": "3168×1440",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "163.1mm",
      "width": "76.8mm",
      "thickness": "8.8mm",
      "weight": "226g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "oppo-find-x8s",
    "brand": "OPPO",
    "model": "OPPO Find X8s",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "小屏旗舰",
      "lightweight",
      "影像"
    ],
    "officialUrl": "https://www.oppo.com/cn/smartphones/series-find-x/find-x8s/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4199,
        "pricePdd": 3999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4699,
        "pricePdd": 4499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5499,
        "pricePdd": 5199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.32英寸",
      "resolution": "2640×1216",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "150.6mm",
      "width": "71.8mm",
      "thickness": "7.8mm",
      "weight": "179g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-x200s",
    "brand": "vivo",
    "model": "vivo X200s",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "直屏旗舰",
      "长续航",
      "影像"
    ],
    "officialUrl": "https://www.vivo.com.cn/products/x200s",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4199,
        "pricePdd": 3999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 4699,
        "pricePdd": 4499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 5199,
        "pricePdd": 4999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸",
      "resolution": "2800×1260",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "160mm",
      "width": "74.3mm",
      "thickness": "7.95mm",
      "weight": "203g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "vivo-x200-ultra",
    "brand": "vivo",
    "model": "vivo X200 Ultra",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "影像机皇",
      "2k屏",
      "长焦"
    ],
    "officialUrl": "https://www.vivo.com.cn/products/x200-ultra",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6499,
        "pricePdd": 6199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 7999,
        "pricePdd": 7699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸",
      "resolution": "3168×1440",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "等深四曲屏"
    },
    "body": {
      "height": "163.3mm",
      "width": "76.7mm",
      "thickness": "8.9mm",
      "weight": "229g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "honor-gt-pro",
    "brand": "荣耀",
    "model": "荣耀 GT Pro",
    "image": "",
    "launchDate": "2024-12",
    "tags": [
      "gaming",
      "大电池",
      "旗舰芯"
    ],
    "officialUrl": "https://www.honor.com/cn/phones/honor-gt-pro/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3199,
        "pricePdd": 2999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3699,
        "pricePdd": 3499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 4299,
        "pricePdd": 3999,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.78英寸",
      "resolution": "2800×1264",
      "refreshRate": "144Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "162.8mm",
      "width": "75.9mm",
      "thickness": "8.6mm",
      "weight": "219g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP69"
    }
  },
  {
    "id": "redmi-turbo-4-pro",
    "brand": "小米",
    "model": "红米 Turbo 4 Pro",
    "image": "",
    "launchDate": "2025-04",
    "tags": [
      "value",
      "超大电池",
      "金属中框"
    ],
    "officialUrl": "https://www.mi.com/redmi-turbo4-pro",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 1999,
        "pricePdd": 1799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 2299,
        "pricePdd": 2099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 2799,
        "pricePdd": 2599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.83英寸",
      "resolution": "2772×1280",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "直屏"
    },
    "body": {
      "height": "164mm",
      "width": "77.5mm",
      "thickness": "8.1mm",
      "weight": "208g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "xiaomi-civi-5-pro",
    "brand": "小米",
    "model": "小米 Civi 5 Pro",
    "image": "",
    "launchDate": "2025-05",
    "tags": [
      "轻薄自拍",
      "颜值",
      "长续航"
    ],
    "officialUrl": "https://www.mi.com/civi5-pro",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2999,
        "pricePdd": 2799,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 3299,
        "pricePdd": 3099,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 3799,
        "pricePdd": 3499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.55英寸",
      "resolution": "2750×1236",
      "refreshRate": "120Hz",
      "material": "LTPO AMOLED",
      "type": "全等深微曲屏"
    },
    "body": {
      "height": "160.4mm",
      "width": "73.2mm",
      "thickness": "7.45mm",
      "weight": "184g",
      "material": "金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "不支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 2.0)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-17",
    "brand": "苹果",
    "model": "iPhone 17",
    "image": "",
    "launchDate": "2025-09",
    "tags": [
      "a19",
      "promotion",
      "lightweight"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-17/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5999,
        "pricePdd": 5699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 7999,
        "pricePdd": 7599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.3英寸",
      "resolution": "2622×1206",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "149.6mm",
      "width": "71.5mm",
      "thickness": "7.95mm",
      "weight": "177g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "8GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-17-pro",
    "brand": "苹果",
    "model": "iPhone 17 Pro",
    "image": "",
    "launchDate": "2025-09",
    "tags": [
      "a19_pro",
      "三摄",
      "promotion"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-17-pro/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 8999,
        "pricePdd": 8599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 10999,
        "pricePdd": 10499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 12999,
        "pricePdd": 12499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.3英寸",
      "resolution": "2622×1206",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "149.6mm",
      "width": "71.5mm",
      "thickness": "8.75mm",
      "weight": "204g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "iphone-17-pro-max",
    "brand": "苹果",
    "model": "iPhone 17 Pro Max",
    "image": "",
    "launchDate": "2025-09",
    "tags": [
      "a19_pro",
      "大屏",
      "长焦"
    ],
    "officialUrl": "https://www.apple.com.cn/iphone-17-pro-max/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 9999,
        "pricePdd": 9599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 11999,
        "pricePdd": 11499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 13999,
        "pricePdd": 13499,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "2868×1320",
      "refreshRate": "120Hz",
      "material": "OLED",
      "type": "直屏"
    },
    "body": {
      "height": "163.4mm",
      "width": "78mm",
      "thickness": "8.75mm",
      "weight": "233g",
      "material": "铝合金中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "不支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.3",
      "usb": "USB-C (USB 3)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s26",
    "brand": "三星",
    "model": "Samsung S26",
    "image": "",
    "launchDate": "2026-01",
    "tags": [
      "骁龙8_elite_gen2",
      "小屏",
      "ai"
    ],
    "officialUrl": "https://www.samsung.com/cn/smartphones/galaxy-s26/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 5999,
        "pricePdd": 5699,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 6999,
        "pricePdd": 6599,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.2英寸",
      "resolution": "2340×1080",
      "refreshRate": "120Hz",
      "material": "Dynamic AMOLED 2X",
      "type": "直屏"
    },
    "body": {
      "height": "146.9mm",
      "width": "70.5mm",
      "thickness": "7.2mm",
      "weight": "172g",
      "material": "装甲铝中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "不支持",
      "esim": "支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "samsung-s26-ultra",
    "brand": "三星",
    "model": "Samsung S26 Ultra",
    "image": "",
    "launchDate": "2026-01",
    "tags": [
      "s_pen",
      "2亿像素",
      "ai"
    ],
    "officialUrl": "https://www.samsung.com/cn/smartphones/galaxy-s26-ultra/",
    "lastVerified": "2026-08-04",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 9699,
        "pricePdd": 9299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": 10699,
        "pricePdd": 10299,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": 12699,
        "pricePdd": 12199,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.9英寸",
      "resolution": "3120×1440",
      "refreshRate": "120Hz",
      "material": "Dynamic AMOLED 2X",
      "type": "直屏"
    },
    "body": {
      "height": "162.8mm",
      "width": "77.6mm",
      "thickness": "8.2mm",
      "weight": "232g",
      "material": "钛金属中框+玻璃"
    },
    "ram": "12GB",
    "features": {
      "expandable_storage": "不支持",
      "dual_sim": "支持",
      "five_g": "支持",
      "nfc": "支持",
      "infrared": "支持",
      "esim": "支持",
      "satellite": "支持",
      "wifi": "Wi-Fi 7",
      "bluetooth": "5.4",
      "usb": "USB-C (USB 3.2)",
      "waterproof": "IP68"
    }
  },
  {
    "id": "google-pixel-9",
    "brand": "谷歌",
    "model": "Pixel 9",
    "image": null,
    "launchDate": "2024-08-13",
    "tags": [
      "flagship",
      "ai",
      "photography",
      "clean_android"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "128GB",
        "priceJd": 3650,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "256GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "128GB",
    "screen": {
      "size": "6.3英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "google-pixel-9-pro-xl",
    "brand": "谷歌",
    "model": "Pixel 9 Pro XL",
    "image": null,
    "launchDate": "2024-08-13",
    "tags": [
      "flagship",
      "ai",
      "photography",
      "clean_android"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4878,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.8英寸"
    },
    "body": {},
    "ram": "16GB",
    "features": {}
  },
  {
    "id": "motorola-edge-50-ultra",
    "brand": "摩托罗拉",
    "model": "Edge 50 Ultra",
    "image": null,
    "launchDate": "2024-04-01",
    "tags": [
      "flagship",
      "fast_charging",
      "clean_android"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 4999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "1TB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "6.7英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "redmi-redmi-k80",
    "brand": "红米",
    "model": "Redmi K80",
    "image": null,
    "launchDate": "2024-11-27",
    "tags": [
      "flagship_killer",
      "value",
      "gaming",
      "high_refresh_rate"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 2499,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.67英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "iqoo-iqoo-13",
    "brand": "iQOO",
    "model": "iQOO 13",
    "image": null,
    "launchDate": "2024-10-30",
    "tags": [
      "flagship",
      "gaming",
      "fast_charging",
      "high_refresh_rate"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.82英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "meizu-meizu-21",
    "brand": "魅族",
    "model": "Meizu 21",
    "image": null,
    "launchDate": "2023-11-30",
    "tags": [
      "flagship",
      "photography",
      "narrow_bezel"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3699,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.55英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "nubia-z70-ultra",
    "brand": "努比亚",
    "model": "Z70 Ultra",
    "image": null,
    "launchDate": "2024-11-21",
    "tags": [
      "flagship",
      "photography",
      "under_display_camera",
      "fast_charging"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4599,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      },
      {
        "storage": "512GB",
        "priceJd": null,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.85英寸"
    },
    "body": {},
    "ram": "12GB",
    "features": {}
  },
  {
    "id": "huawei-mate-x7",
    "brand": "华为",
    "model": "Mate X7",
    "image": null,
    "launchDate": "2025-12-05",
    "tags": [
      "foldable",
      "flagship",
      "harmonyos",
      "5g"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 12999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "主:8英寸; 副:6.49英寸",
      "resolution": "主:2416x2210px; 副:2440x1080px",
      "refreshRate": "120Hz (1-120)",
      "material": "主:OLED(LTPO); 副:OLED(LTPO)",
      "type": "折叠屏 OLED (挖孔屏)"
    },
    "body": {
      "height": "156.8mm (展开/折叠 同)",
      "width": "144.2mm (展开) / 73.8mm (折叠)",
      "thickness": "4.5mm (展开) / 9.5mm (折叠)",
      "weight": "235g",
      "material": "铝合金边框, 素皮后壳"
    },
    "ram": "12GB / 16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙6.0 (支持 LDAC, L2HC)",
      "wifi": "Wi-Fi 6/7 (802.11a/b/g/n/ac/ax/be)",
      "nfc": "支持",
      "infrared": "支持",
      "usb": "USB Type-C (USB 3.1 Gen1, 支持 DP1.2)",
      "waterproof": "IP58 / IP59",
      "satellite": "北斗卫星消息",
      "esim": null,
      "expandable_storage": null
    }
  },
  {
    "id": "honor-magic-v5",
    "brand": "荣耀",
    "model": "Magic V5",
    "image": null,
    "launchDate": "2025-07-02",
    "tags": [
      "foldable",
      "flagship",
      "magicos",
      "5g"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 8999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "主:7.95英寸; 副:6.43英寸",
      "resolution": "主:2352x2172px; 副:2376x1060px",
      "refreshRate": "120Hz",
      "material": "主:柔性OLED; 副:OLED(LTPO)",
      "type": "折叠屏 OLED (挖孔屏)"
    },
    "body": {
      "height": "156.8mm (展开/折叠 同)",
      "width": "145.9mm (展开) / 74.3mm (折叠)",
      "thickness": "4.1mm (展开, 暖白) / 4.2mm (展开, 其他) / 8.8mm (折叠, 暖白) / 9.0mm (折叠, 其他)",
      "weight": "217g (暖白) / 222g (其他配色)",
      "material": "铝合金中框, 玻璃/素皮后壳"
    },
    "ram": "12GB / 16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙6.0 (支持 LDAC, aptX 系列)",
      "wifi": "Wi-Fi 6/7 (802.11 ax/be)",
      "nfc": "支持",
      "infrared": "支持",
      "usb": "USB Type-C (USB 3.1 Gen1, 支持 DP1.2)",
      "waterproof": "IP58 / IP59",
      "satellite": "北斗卫星通信",
      "esim": null,
      "expandable_storage": null
    }
  },
  {
    "id": "samsung-galaxy-z-trifold",
    "brand": "三星",
    "model": "Galaxy Z TriFold",
    "image": null,
    "launchDate": "2025-12-09",
    "tags": [
      "foldable",
      "trifold",
      "flagship",
      "oneui",
      "5g"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "512GB",
        "priceJd": 19999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "512GB",
    "screen": {
      "size": "主:10英寸; 副:6.5英寸",
      "resolution": "主:2160x1584px; 副:2520x1080px",
      "refreshRate": "120Hz",
      "material": "第二代动态AMOLED",
      "type": "三折叠屏 AMOLED (挖孔屏)"
    },
    "body": {
      "height": "159.2mm (展开/折叠 同)",
      "width": "214.1mm (展开) / 75.0mm (折叠)",
      "thickness": "3.9mm (展开) / 12.9mm (折叠)",
      "weight": "309g",
      "material": "装甲铝边框, 陶瓷玻纤背板"
    },
    "ram": "16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙5.4",
      "wifi": "Wi-Fi 6/7 (802.11a/b/g/n/ac/ax/be)",
      "nfc": "支持",
      "infrared": null,
      "usb": "USB Type-C (USB 3.2 Gen1)",
      "waterproof": "IP48",
      "satellite": "天通卫星通讯 (语音/短信)",
      "esim": null,
      "expandable_storage": null
    }
  },
  {
    "id": "vivo-x-fold5",
    "brand": "vivo",
    "model": "X Fold5",
    "image": null,
    "launchDate": "2025-07-02",
    "tags": [
      "foldable",
      "flagship",
      "zeiss",
      "originos",
      "5g"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 6999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "主:8.03英寸; 副:6.53英寸",
      "resolution": "主:2480x2200px; 副:2748x1172px",
      "refreshRate": "120Hz",
      "material": "AMOLED",
      "type": "折叠屏 AMOLED (挖孔屏, 曲面)"
    },
    "body": {
      "height": "159.68mm (展开/折叠 同)",
      "width": "142.29mm (展开) / 72.60mm (折叠)",
      "thickness": "4.55mm (展开, 明白/青松) / 4.3mm (展开, 钛度) / 9.7mm (折叠, 明白/青松) / 9.2mm (折叠, 钛度)",
      "weight": "226g (明白/青松) / 217g (钛度)",
      "material": "铝合金中框"
    },
    "ram": "12GB / 16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙5.4 (支持 LDAC, LHDC 5.0)",
      "wifi": "Wi-Fi 6/7 (802.11a/b/g/n/ac/ax/be)",
      "nfc": "支持 (HCE/eSE 支付)",
      "infrared": "支持",
      "usb": "USB Type-C (USB 3.2 Gen1, 支持 DP 投屏)",
      "waterproof": "IPX8 / IPX9",
      "satellite": null,
      "esim": null,
      "expandable_storage": null
    }
  },
  {
    "id": "nubia-11-pro",
    "brand": "努比亚",
    "model": "红魔11 Pro",
    "image": null,
    "launchDate": "2025-10-17",
    "tags": [
      "gaming",
      "flagship",
      "5g",
      "active-cooling"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 4999,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.85英寸",
      "resolution": "2688x1216px (1.5K)",
      "refreshRate": "144Hz",
      "material": "京东方 X10 OLED (屏下真全面屏)",
      "type": "OLED 真全面屏 (屏下摄像头, 无挖孔)"
    },
    "body": {
      "height": "163.82mm",
      "width": "76.54mm",
      "thickness": "8.9mm",
      "weight": "230g",
      "material": "铝合金中框, 透明玻璃/素皮后壳"
    },
    "ram": "12GB / 16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙5.4",
      "wifi": "Wi-Fi 6/7 (802.11a/b/g/n/ac/ax/be)",
      "nfc": "支持 (全功能 NFC)",
      "infrared": "支持 (红外遥控)",
      "usb": "USB Type-C",
      "waterproof": "IPX8",
      "satellite": null,
      "esim": null,
      "expandable_storage": null
    }
  },
  {
    "id": "nubia-11-air",
    "brand": "努比亚",
    "model": "红魔11 Air",
    "image": null,
    "launchDate": "2026-01-20",
    "tags": [
      "gaming",
      "thin-light",
      "5g",
      "active-cooling"
    ],
    "officialUrl": null,
    "lastVerified": "2026-08-10",
    "storageOptions": [
      {
        "storage": "256GB",
        "priceJd": 3499,
        "pricePdd": null,
        "shopTypeJd": "官方旗舰店",
        "shopTypePdd": "品牌",
        "couponNote": ""
      }
    ],
    "defaultStorage": "256GB",
    "screen": {
      "size": "6.85英寸",
      "resolution": "2688x1216px (1.5K)",
      "refreshRate": "144Hz",
      "material": "京东方 Q10 OLED (悟空屏, 屏下真全面屏)",
      "type": "OLED 真全面屏 (屏下摄像头, 无挖孔)"
    },
    "body": {
      "height": "163.82mm",
      "width": "76.54mm",
      "thickness": "7.85mm",
      "weight": "207g",
      "material": "金属中框, 透明玻璃后盖"
    },
    "ram": "12GB / 16GB",
    "features": {
      "dual_sim": "支持 (双 Nano SIM)",
      "five_g": "支持 (移动/联通/电信/广电 5G)",
      "bluetooth": "蓝牙5.4",
      "wifi": "Wi-Fi 6/7 (802.11a/b/g/n/ac/ax/be)",
      "nfc": "支持 (全功能 NFC)",
      "infrared": "支持 (红外遥控)",
      "usb": "USB Type-C",
      "waterproof": null,
      "satellite": null,
      "esim": null,
      "expandable_storage": null
    }
  }
];
